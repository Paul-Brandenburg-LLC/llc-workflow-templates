...
## Tor-Ausweichung: distribution/ + Codex-Ausfall (Standard 8.1, Kapitel 5)
`distribution/` ist der ZWEITE Verteilweg neben dem Reusable-Pin: ganze Dateien, erkannt am Marker `llc-verteilung: verwaltet` IM ZIEL. Noetig fuer das Box-Tor, weil dessen Bruecke als Datei im Consumer liegen muss (`python3 .github/scripts/entwicklung_review_bridge.py`) — dorthin traegt kein `uses:`. Workflow **und** Bruecke gehen in EINEM PR; getrennt entstuende ein Fenster, in dem eine tote Pflichtpruefung steht.
`gate-2-codex.yml` weicht bei Codex-Ausfall auf den Box-Pruefer aus (Step `urteil`, Funktion `ausweich_urteil()`). Getragen wird das allein von den HEAD-gebundenen Marken im Status-Text des Box-Tors: `[llc-tor:<vendor>/<verdikt>]` und `[llc-ausfall:<vendor>/<grund>]`. **⛔ `entwicklung-review` steht auch bei „Prüfer A ausgefallen" auf `success` (fail-open)** — ohne die `approve`-Marke waere jeder Box-Ausfall eine Freigabe fuer Codex. Deshalb nie auf den Status-State allein pruefen. Beweis: `scripts/codex-ausweichung-selftest.sh`, Faelle A3/A4/B4.
**⛔ `health` steht NICHT in der Allowlist der Box** (`llc-ops-backlog/deploy/entwicklung-box/entwicklung-release-control` kennt nur `review-start`, `review-result`, `deploy`, `rollback`). Die Bruecke ruft es tolerant auf und faellt auf `review-result.reason` zurueck. Wer `vendor_blocks` auf GitHub-Ebene braucht, muss zuerst diese Zeile ergaenzen.
